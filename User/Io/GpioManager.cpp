...
void GpioManager::IoClock(void *m_Periph, bool v) {
...
//    TRegGpio(TouchKey,128,5);   //触摸案列
 //   TRegGpio(LED,128,5);  //闪烁案列
    TRegGpio(LUpWheel,128,5);  //前左轮
    TRegGpio(RUpWheel,128,5);  //前右轮
    TRegGpio(LDpWheel,128,5);  //后左轮
    TRegGpio(RDpWheel,128,5);  //后右轮
...
void GpioManager::AddGpio(Gpio* m_Gpio) {
